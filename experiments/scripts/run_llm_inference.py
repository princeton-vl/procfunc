...
# API key is read from --api_key_txt file, or falls back to OPENAI_API_KEY / GOOGLE_API_KEY env vars
...
def get_render_path(render_dir: Path) -> Path | None:
    """Get render from a directory. Concatenates render1.png, render2.png, etc. to render.png."""
    render_path = render_dir / "render.png"
    numbered = sorted(render_dir.glob("render[0-9]*.png"))
    if render_path.exists():
        assert not numbered, f"render.png exists but numbered renders also found: {numbered}"
        return render_path
    if not numbered:
        return None
    if len(numbered) == 1:
...
def log_backoff(details):
    if details['wait'] >= 30:
        logger.warning(f"[RATELIMIT] attempt {details['tries']}, retrying in {details['wait']:.1f}s...")

def log_giveup(details):
    logger.warning(f"[RATELIMIT] giving up after {details['tries']} attempts, {details['elapsed']:.1f}s elapsed")
...
@backoff.on_exception(backoff.expo, (ResourceExhausted, GenaiClientError, GenaiServerError), on_backoff=log_backoff, max_value=300)
...
@backoff.on_exception(backoff.expo, openai.RateLimitError, max_time=300, on_backoff=log_backoff)
def call_openai(client, model: str, parts: list) -> str:
    """Send message to OpenAI. parts is list of str or Path."""
    client["messages"].append({"role": "user", "content": _content_for_openai(parts)})
...
def render_pf(code_path: Path, render_dir: Path, blendfile: Path) -> tuple[Path | str, list[str]]:
    ...
    render_path = get_render_path(render_dir)
...
def run_round(
    ...
    log = lambda msg: logger.info(f"[{task}][r{round_idx}]{msg}")
    logwarn = lambda msg: logger.warning(f"[{task}][r{round_idx}]{msg}")
    ...
    goal_render = get_render_path(goal_render_dir)
    ...
    with render_lock():
        if args.backend == "pf":
            render_result, render_cmd = render_pf(code_path, render_dir, blendfile)
        elif args.backend == "bgym":
            render_result, render_cmd = render_bgym(
                code_path, render_dir, blendfile, args.blender_path, args.render_script
            )
        else:
...
def init_client(model: str, api_key_txt: Path | None = None):
    ...
    if api_key_txt is not None:
        api_key = api_key_txt.read_text().strip()
...
def run_task(args, task_name: str) -> list[dict]:
    ...
    current_code, last_result = get_start(task_folder)
    chat_data = []
    crashes = []
