...
```bash
uv run python scripts/generate_benchdata_procfunc.py outputs/bench_data outputs/bench_data_pf --task_type material --blender_bgym_path blender_3_6/blender
uv run python scripts/generate_benchdata_procfunc.py outputs/bench_data outputs/bench_data_pf --task_type geometry --blender_bgym_path blender_3_6/blender
```
...
blender_3_6/blender --background --python scripts/create_blender_nodes_interface.py -- prompts/reference_code/ifg_shader_nodes_interface.txt --node-types ShaderNode 
blender_3_6/blender --background --python scripts/create_blender_nodes_interface.py -- prompts/reference_code/ifg_geo_nodes_interface.txt --node-types GeometryNode ShaderNode FunctionNode
```
...
# infinigen v1 examples
cat $INCONTEXT_FILES | grep -v import > prompts/reference_code/infinigenv1_incontext_examples.py

# procfunc examples
uv run python scripts/executable_materials_to_procfunc.py prompts/reference_code/pf_incontext_examples/ outputs/bench_data_pf_fromscratch/material1/blender_file.blend $INCONTEXT_FILES --blender_bgym_path blender_3_6/blender
cat prompts/reference_code/pf_incontext_examples/*.py | grep -v import > prompts/reference_code/pf_incontext_examples.py
```

Generate in-context geometry examples:
```bash
TODO TODO
```
...
Create a `credentials/` directory and write your API keys into text files:
```bash
mkdir -p credentials
echo "your-openai-key-here" > credentials/openai_api.txt
echo "your-google-key-here" > credentials/gemini_api.txt
```
(Alternatively, you can `export OPENAI_API_KEY=...` and `export GOOGLE_API_KEY=...` as environment variables)
