...
def to_environment(
    ...
    outputs = unwrap_and_drop({"surface": surface, "volume": volume})
...
def to_light(
    ...
    outputs = unwrap_and_drop({"surface": surface})
...
def to_mesh_object(
    ...
    try:
        obj_result = extract_geometry_singlekey(
            ng,
            "geometry",
            attribute_keys=[],
            realize=True,
...
def to_objects_multi(
    ...
    all_ng_outputs = unwrap_and_drop(all_ng_outputs)  # type: ignore
    graph = cg.ComputeGraph(
        inputs=pytree.PyTree({}),
    ...
    for k, v in geometries.items():
        ...
        result_objects[k] = extract_geometry_singlekey(
            nodegroup,
            k,
