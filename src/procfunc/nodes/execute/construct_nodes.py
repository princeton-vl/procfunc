...
def connect_single_input(
    ...
    match input_val:
        ...
        case bpy.types.NodeInternal() as from_bpy_node:
            raise ValueError(
...
def _construct_inputs(
    ...
    for k, input_val in node.kwargs.items():
        input_results[k] = construct_input(input_val, bl_node_tree, cache)
    for i, input_val in enumerate(node.args):
        input_results[i] = construct_input(input_val, bl_node_tree, cache)
