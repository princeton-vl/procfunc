...
class ProcNode(Generic[T]):
    """
    A class for programmatic construction of blender-like shader/geometry nodegraphs

    ProcNode provides dunders to compactly emulate a nodegraph, e.g. __getattr__ and __sum

    ProcNode is type-parameterized on what the result type is - typically either a single socket-like type (e.g. Float), or sometimes a NamedTuple for multioutput sockets.
    ...
    def astype(self, dtype: type) -> "ProcNode":
        ...
        Currently this just adds an annotation to help subsequent type-inferred functions/operators
    ...
    def output_socket(self, name: str) -> "ProcNode":
