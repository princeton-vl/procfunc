...
def _codegen_for_graph(
    ...
    fold_map = _expression_fold_map(graph, scope_expressions, usages)
    expressions = _expressions_scope_for_graph(graph, scope_expressions, fold_map)
    _input_ids = set(id(node) for node in graph.inputs.values())  # noqa: F841
    output_ids = {id(node) for node in graph.outputs.values()}
    last_varname: str = ""
    ...
    for node in cg.traverse_depth_first(graph):
        ...
        node_usages = usages.get(id(node), [])
        mutation_usages = [
            usage
            for usage in node_usages
            if isinstance(usage, cg.MutatedArgumentNode) and usage.args[1] is node
        ]
        if (
            mutation_usages
            and mutation_usages == node_usages
