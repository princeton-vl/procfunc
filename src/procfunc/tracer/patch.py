...
def _targets_from_module(
    ...
    We will allow them to be executed if they are called with non-dynamic args. TODO this is potentially unsafe for bpy

    Args:
        module: the module to gather targets from
        seen: a set of ids of functions that have already been processed and should be skipped. TODO potentially redundant
...
class Patcher:
    ...
    def apply_preexecute_patches(
        ...
        for target in self.patch_function_ids.values():
            ...
            if orig_fn is None:
                raise NotImplementedError(
                    f"{target.name} not found in frame, maybe its a builtin? TODO"
                )
